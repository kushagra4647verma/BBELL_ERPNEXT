...
frappe.ui.form.on('Bank Statement Transaction Entry', {
	setup: function(frm) {
		frm.get_field("payment_invoice_items").grid.cannot_add_rows = true;
	...
	remove_unallocated_invoices: function(frm) {
		frm.events.invoke_doc_function(frm, "remove_unallocated_entries");
		frm.refresh_field("payment_invoice_items");
		cur_frm.reload_doc()
	},
});
