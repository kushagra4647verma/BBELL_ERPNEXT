...
def get_report_pdf(doc, consolidated=True):
	...
	template_path = (
		"finbyzerp/public/html/process_statement_of_accounts.html"
	)
	...
	for entry in doc.customers:
		...
		tax_id = frappe.get_doc(doctype, type).tax_id
		presentation_currency = (
			get_party_account_currency(doctype, type, doc.company)
			or doc.currency
		...
		html = frappe.render_template(
			base_template_path,
			{"body": html, "css": get_print_style(), "title": "Statement For " + type},
		)
		statement_dict[type] = html
...
def get_supplier_based_on_territory_or_customer_group(customer_collection, collection_name):
	fields_dict = {
...
def fetch_customers(customer_collection, collection_name, primary_mandatory):
	...
	if customer_collection == "Sales Person":
		...
		elif customer_collection == "Supplier Group":
			customers = get_supplier_based_on_territory_or_customer_group(
	...
	for customer in customers:
		...
		if customer_collection == "Supplier Group":
			type_of = "Supplier"
		else:
			type_of = "Customer"
		billing_email = get_customer_emails(type_of,customer.name, 1, billing_and_primary=False)
		primary_email = None
		...
		customer_list.append(
			{"type": type_of,"name": customer.name, "primary_email": primary_email, "billing_email": billing_email}
		)
	return customer_list
	

@frappe.whitelist()
def get_customer_emails(type, customer_name, primary_mandatory, billing_and_primary=True):
	"""Returns first email from Contact Email table as a Billing email
	when Is Billing Contact checked
	and Primary email- email with Is Primary checked"""

	billing_email = frappe.db.sql(f"""
		SELECT
			email.email_id
		FROM
		...
		WHERE
			link.link_doctype='{type}'
			and link.link_name='{customer_name}'
...
def download_statements(document_name):
	doc = frappe.get_doc("Statement of Accounts", document_name)
	...
	if report:
		for customer, report_pdf in report.items():
			...
			frappe.enqueue(
				...
				message=message,
				now=True,
				reference_doctype="Statement of Accounts",
