...
def get_sle_value(filters):
	...
	if filters.get("account"):
		warehouse_list = frappe.db.get_all("Warehouse",{"account":filters.get("account")}, pluck= "name")
		warehouse_placeholder = ', '.join(f"'{i}'" for i in warehouse_list)
		conditions += """ and sle.warehouse in (%s)""" % warehouse_placeholder
...
def get_gl_value(filters):
	...
	if filters.get("account"):
		conditions += " and gl.account = %s" % frappe.db.escape(filters.get("account"))
	else:
		conditions += """ and ac.account_type in ("Stock","Capital Work in Progress","Fixed Asset")"""
	...
	gl_data = frappe.db.sql("""
			...
			where gl.docstatus < 2 and gl.is_cancelled = 0 %s
