...
def _execute(filters=None):
	...
	if item_list:
		itemised_tax, tax_columns = get_tax_accounts(filters, item_list, columns, company_currency)
...
# and `tabSales Taxes and Charges`.account_head like '%%GST%%'
...
def get_items(filters):
	...
	# 	from
	# 		`tabSales Invoice`
	# 		LEFT JOIN `tabSales Taxes and Charges` on `tabSales Taxes and Charges`.parent = `tabSales Invoice`.name and `tabSales Taxes and Charges`.tax_amount != 0
	# 		LEFT JOIN `tabSales Invoice Item` on `tabSales Invoice`.name = `tabSales Invoice Item`.parent and `tabSales Invoice Item`.gst_hsn_code is not NULL
	# 	where
...
def get_tax_conditions(filters):
	conditions = " and `tabSales Taxes and Charges`.account_head LIKE '%%GST%%'"
...
def get_tax_accounts(
	...
	tax_conditions = get_tax_conditions(filters)
	match_conditions = frappe.build_match_conditions("Sales Invoice")
	if match_conditions:
		match_conditions = " and {0} ".format(match_conditions)

	tax_query = frappe.db.sql(
		"""
		select
			`tabSales Taxes and Charges`.item_wise_tax_detail, `tabSales Taxes and Charges`.parent, `tabSales Taxes and Charges`.name as tax_name
		from
			`tabSales Invoice`,
			`tabSales Taxes and Charges`
		where
			`tabSales Taxes and Charges`.parent = `tabSales Invoice`.name
			and `tabSales Taxes and Charges`.tax_amount != 0
			and `tabSales Invoice`.docstatus = 1 %s %s
		"""
		% (tax_conditions, match_conditions),
		filters,
		as_dict=1,
	)
	...
	tax_dict = {}
	tax_count_dict = {}
	for tax_row in tax_query:
		tax_dict[tax_row.parent] = tax_row
		if tax_count_dict.get(tax_row.parent):
			tax_count_dict[tax_row.parent] += 1
		else:
			tax_count_dict[tax_row.parent] = 1

	for d in item_list:
		tax_data = tax_dict.get(d.parent)
		
		if tax_data and tax_data.get('item_wise_tax_detail') and d.base_net_amount != 0:
			tax_item_wise = json.loads(tax_data.get('item_wise_tax_detail'))
			d['tax_rate'] = (tax_item_wise.get(d.item_code)[0] or 0) * (tax_count_dict.get(d.parent) or 0)
		else:
			d['tax_rate'] = 0
...
def get_hsn_wise_json_data(filters, report_data):
	...
	gst_accounts = get_gst_accounts_by_type(filters.company)
	data = []
	count = 1
