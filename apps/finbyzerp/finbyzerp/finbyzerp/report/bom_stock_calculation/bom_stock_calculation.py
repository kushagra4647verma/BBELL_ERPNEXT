...
#	if not filters: filters = {}
	...
	for row in data:
        # Finbyz Changes START: Changes as per BOM Qty
		reqd_qty = qty_to_make * row.qty if row.get('qty') else qty_to_make * row.stock_qty
        # Finbyz Changes END
...
def get_report_data(last_pur_price, reqd_qty, row, manufacture_details):
	...
	diff_qty = row.actual_qty - reqd_qty
    # Finbyz Changes:   Changes as per BOM Qty: row.get('qty') or row.get('stock_qty')
	return [row.item_code, row.description,
		...
		row.get('qty') or row.get('stock_qty'), row.actual_qty,
...
def get_columns():
	...
	columns = [
		...
		# {"label": _("Manufacturer"), "fieldname": "manufacturer", "fieldtype": "Data","width": 250},
		# {"label": _("Manufacturer Part Number"), "fieldname": "manufacturer_part", "fieldtype": "Data","width": 250},
		{"label": _("Qty"), "fieldname": "qty", "fieldtype": "Float","width": 120},
		{"label": _("Stock Qty"), "fieldname": "actual_qty", "fieldtype": "Float","width": 140},
		{"label": _("Reqd Qty"), "fieldname": "reqd_qty", "fieldtype": "Float","width": 120},
		{"label": _("Diff Qty"), "fieldname": "diff_qty", "fieldtype": "Float","width": 140},
		{"label": _("Last Purchase Price"), "fieldname": "last_pur_price", "fieldtype": "Float","width": 120}
...
def get_bom_stock(filters):
	...
	if not bom:
		frappe.throw("Please Select BOM")
	...
	return frappe.db.sql("""
			...
			WHERE
				bom_item.parent in {bom} and bom_item.parenttype='BOM'

			GROUP BY bom_item.item_code""".format(qty_field=qty_field, table=table, conditions=conditions, bom=list_to_sql(bom)), as_dict=1)
