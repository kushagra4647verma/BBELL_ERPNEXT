...
def execute(filters=None):
	...
	return BillWiseLedger(filters).run(args)


class BillWiseLedger(object):
	def __init__(self, filters=None):
		self.filters = frappe._dict(filters or {})
		self.additional_dict = frappe._dict({})
	...
	def set_defaults(self):
		...
		if self.filters.get('show_all_invoices'):
			if self.party_type == "Customer" and not self.filters.get('customer'):
				frappe.throw("Customer is Mandatory")
			elif self.party_type == "Supplier" and not self.filters.get('supplier'):
	...
	def get_data(self):
		...
		self.remaining_voucher_balance = OrderedDict()
	...
	def init_voucher_balance(self):
		...
		for gle in self.gl_entries:
			...
			if gle.against_voucher and not (gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher) in self.remaining_voucher_balance:
				self.remaining_voucher_balance[(gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)] = frappe._dict(
					indent = 1,
					...
					credit_note=0.0,
					outstanding=0.0,
					against_voucher_type = gle.get('against_voucher_type') or '',
					against_voucher = gle.get('against_voucher') if gle.voucher_no != gle.against_voucher else '',
	...
	def update_voucher_balance(self, gle):
		...
		if gle_balance > 0:
			if gle.voucher_type in ("Journal Entry", "Payment Entry") and gle.against_voucher:
				...
				if self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)):
					self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher))['paid'] = abs(gle_balance)
					self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher))['paid_in_account_currency'] = abs(gle_balance_in_account_currency)

				if gle.against_voucher != gle.voucher_no and gle.against_voucher_type not in ("Journal Entry", "Payment Entry"):
					if (gle.against_voucher_type, gle.against_voucher) not in self.additional_dict.keys():
						self.additional_dict.update({(gle.against_voucher_type, gle.against_voucher) : [self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher))]})
						del self.remaining_voucher_balance[(gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)]
					else:
						self.additional_dict[(gle.against_voucher_type, gle.against_voucher)].append(self.voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)))
						del self.remaining_voucher_balance[(gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)]
			else:
				# invoice
			...
			if self.is_invoice(gle):
				...
				if self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)):
					self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher))['credit_note'] = abs(gle_balance)
					self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher))['credit_note_in_account_currency'] = abs(gle_balance_in_account_currency)
			else:
			...
			if gle.against_voucher:
				if (gle.against_voucher_type, gle.against_voucher) not in self.additional_dict.keys():
					...
					if gle.against_voucher != gle.voucher_no and gle.against_voucher_type not in ("Journal Entry", "Payment Entry"):
						self.additional_dict[(gle.against_voucher_type, gle.against_voucher)].append(self.remaining_voucher_balance.get((gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)))
						del self.remaining_voucher_balance[(gle.voucher_type, gle.voucher_no, gle.party, gle.against_voucher)]
	...
	def build_data(self):
		...
		for key, row in self.voucher_balance.items():
			...
			row.indent = 0
			if self.additional_dict.get((row.voucher_type, row.voucher_no)):
				row.paid = 0
				if row.invoiced != 0:
					row.credit_note = 0
			row.invoice_grand_total = row.invoiced

			if not self.filters.get('show_all_invoices') and (abs(row.outstanding) > 1.0 / 10**self.currency_precision) and (
				...
				if self.is_invoice(row) and self.filters.based_on_payment_terms:
					...
					self.append_row(row)
					if self.additional_dict.get((row.voucher_type, row.voucher_no)) and row.voucher_type not in ("Journal Entry", "Payment Entry"):
						for row in self.additional_dict.get((row.voucher_type, row.voucher_no)):
							if row:
								self.append_row(row)
			elif self.filters.get('show_all_invoices'):
				if self.is_invoice(row) and self.filters.based_on_payment_terms:
					# is an invoice, allocate based on fifo
					# adds a list `payment_terms` which contains new rows for each term
					self.allocate_outstanding_based_on_payment_terms(row)
					...
					if not self.additional_dict.get(row.against_voucher_type, row.against_voucher):
						self.append_row(row)
					if self.additional_dict.get((row.voucher_type, row.voucher_no)) and row.voucher_type not in ("Journal Entry", "Payment Entry"):
						for row in self.additional_dict.get((row.voucher_type, row.voucher_no)):
							if row:
	...
	def get_future_payments_from_journal_entry(self):
		if self.filters.get("party"):
			...
			amount_field = "jea.debit - " if self.filters.party_type == "Supplier" else "jea.credit"
		...
		return frappe.db.sql(
			...
			(self.filters.posting_date, self.filters.party_type),
			as_dict=1,
		)
