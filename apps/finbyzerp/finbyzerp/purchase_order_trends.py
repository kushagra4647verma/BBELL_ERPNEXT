...
def purchse_order_trends_execute(filters=None):
    if not filters:
        filters = {}
    data = []
...
def get_chart_data(data, filters):
    ...
    if len(data) > 10:
        # get top 10 if data is too long
        data = data[:10]
    ...
    return {
        "data": {
            "labels": labels,
            "datasets": [{"name": _("Total Ordered Amount"), "values": datapoints}],
        },
        "type": "bar",
        "colors": ["#5e64ff"],
        "fieldtype": "Currency",
    ...
    if filters.get("group_by"):
        ...
        data1 = frappe.db.sql(""" select %s from `tab%s` t1, `tab%s Item` t2 %s
                    ...
                    """ % (query_details, conditions["trans"], conditions["trans"], conditions["addl_tables"], "%s",
                           posting_date, "%s", "%s", conditions.get("addl_tables_relational_cond"), cond, parent_condition,
                           conditions["group_by"]), (filters.get("company"),
            ...
            row = frappe.db.sql("""select DISTINCT(%s) from `tab%s` t1, `tab%s Item` t2 %s
                            ...
                            """ %
                                (sel_col, conditions["trans"], conditions["trans"], conditions["addl_tables"],
                                 "%s", posting_date, "%s", "%s", conditions["group_by"], "%s",
                                 conditions.get("addl_tables_relational_cond"), cond),
            ...
            for i in range(len(row)):
                ...
                row1 = frappe.db.sql(""" select %s , %s from `tab%s` t1, `tab%s Item` t2 %s
                                    ...
                                    (sel_col, conditions["period_wise_select"], conditions["trans"],
                                     conditions["trans"], conditions["addl_tables"], "%s", posting_date, "%s", "%s",
                                     sel_col,
        ...
        data = frappe.db.sql(""" select %s from `tab%s` t1, `tab%s Item` t2 %s
                    ...
                    """ %
                            (query_details, conditions["trans"], conditions["trans"], conditions["addl_tables"],
                             "%s", posting_date, "%s", "%s", cond, conditions.get("addl_tables_relational_cond", ""),
                             parent_condition, conditions["group_by"]),
