frappe.ui.form.on('Account',{
    refresh: function(frm){
        if (frm.doc.name && frm.doc.account_type=='Expense Account'){
            frm.add_custom_button(__("Create Item"), function(frm){
                frappe.call({
                    method:"finbyzerp.finbyzerp.doc_events.account.create_item",
                    args:{
                    ...
                    callback: function(r){
                        ...
                        if (val.date_wise_balance){
                            val.date_wise_balance.forEach(function (row){
                                interest_amount += (row.balance * row.days * val.interest_rate / (365 * 100))
                            })
                            d.set_value('interest_amount',interest_amount)
                        }
                        var numItems = $('.grid-download').length; 
