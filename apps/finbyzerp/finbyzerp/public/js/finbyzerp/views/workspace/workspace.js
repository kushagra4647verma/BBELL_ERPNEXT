...
frappe.views.Workspace = class Workspace {
	...
	prepare_container() {
        ...
        `).appendTo(this.wrapper.find(".icon-side-section"));
        this.sidebar_icon = list_icon_sidebar.find(".desk-icon-sidebar");
	...
	get_pages() {
		return frappe.xcall("finbyzerp.api.get_workspace_sidebar_items");
	}
	...
	sidebar_item_container(item) {
		return $(`
			...
			item.parent_page
				...
				<div class="desk-sidebar-item tets standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${
							item.public
					...
					>
					${ item.module_icon ? `<img src="${ item.module_icon }" alt="${item.title}_icon" class="min_height_width" />` : `<span class="sidebar-item-icon ${ item.title || "folder-normal"}-sidebar-icon" item-icon=${item.icon || "folder-normal"}>${frappe.utils.icon(item.icon || "folder-normal","md")}</span>`}
	...
	build_sidebar_section(title, root_pages) {
		...
		let sidebar_icon_section = $(
			`<div class="standard-sidebar-icon-section nested-container" data-title="${title}"></div>`
		);
		...
		let $title_icon = $(`<div class="standard-sidebar-label">
			<span>${frappe.utils.icon("small-down", "xs")}</span>
			<span class="section-title">${__(title)}<span>
		</div>`).appendTo(sidebar_icon_section);
		this.prepare_sidebar(root_pages, sidebar_icon_section, this.sidebar_icon);
	...
	edit_page(item) {
		...
		const d = new frappe.ui.Dialog({
			...
			fields: [
				...
				{
					label: __("Module Icon"),
					fieldtype: "Attach Image",
					fieldname: "module_icon",
					default: item.module_icon,
				}
			],
			...
			primary_action: (values) => {
				...
				frappe.call({
					method: "finbyzerp.api.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || "",
	...
	duplicate_page(page) {
		...
		const d = new frappe.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				frappe.call({
					method: "finbyzerp.api.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values,
					},
